...
from common import ret_basis, generate_monomials, kernel_constraints

num_tests = 100                     # Number of numerical tests

n = 2
max_degree = [4,3]
alpha, terms_by_degree = generate_monomials(n,max_degree)

initial_state = [ np.random.rand() for _ in range(n) ]
kernel = Kernel(dim=n, degree = max_degree)
